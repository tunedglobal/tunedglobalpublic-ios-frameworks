...
///
/// \param automixIQTunedRestDelegate The delegate in charge of feeding and renewing Tuned Global OAuth2 access tokens. See AutomixIQTunedRestDelegate.
///
...
///
///
+ (void)initialiseWithConfig:(AutomixIQConfig * _Nonnull)config automixIQTunedRestDelegate:(id <AutomixIQTunedRestDelegate> _Nonnull)automixIQTunedRestDelegate automixIQStatusDelegate:(id <AutomixIQStatusObserverDelegate> _Nullable)automixIQStatusDelegate musicCacheDelegate:(id <AutomixIQTunedMusicCacheDelegate> _Nullable)musicCacheDelegate completion:(void (^ _Nullable)(NSError * _Nullable))completion;
...
///
- (void)setMusicCacheDelegate:(id <AutomixIQTunedMusicCacheDelegate> _Nonnull)musicCacheDelegate;
...
///
/// \param metadata An array of AutomixIQTrackModel objects you can pass to the engine so it doesn’t make additional API requests for the corresponding track identifiers metadata. Can be nil.
///
...
///
///
- (void)automixOnIdentifiers:(NSArray<NSString *> * _Nonnull)identifiers metadata:(NSArray<AutomixIQTrackModel *> * _Nullable)metadata playTrackIndex:(int32_t)playTrackIndex seconds:(float)seconds autoplay:(BOOL)autoplay contextType:(NSString * _Nullable)contextType contextId:(NSString * _Nullable)contextId completion:(void (^ _Nullable)(NSError * _Nullable))completion;
...
///
///
- (void)loadTracksWithIdentifiers:(NSArray<NSString *> * _Nonnull)identifiers metadata:(NSArray<AutomixIQTrackModel *> * _Nullable)metadata playTrackIndex:(int32_t)playTrackIndex seconds:(float)seconds autoplay:(BOOL)autoplay contextType:(NSString * _Nullable)contextType contextId:(NSString * _Nullable)contextId completion:(void (^ _Nullable)(NSError * _Nullable))completion;
...
///
///
- (void)addTracksWithIdentifiers:(NSArray<NSString *> * _Nonnull)identifiers metadata:(NSArray<AutomixIQTrackModel *> * _Nullable)metadata index:(int32_t)index completion:(void (^ _Nullable)(NSError * _Nullable))completion;
/// Adds a single track to the AutomixIQ playlist at a given index.
/// Returns an erorr if the index is out of bounds.
/// \param identifier The track identifier to be added.
///
/// \param metadata AutomixIQTrackModel object you can pass to the engine so it doesn’t make additional API requests for the corresponding track identifier metadata. Can be nil.
///
/// \param index The index at which this track will be added. If index == 0, it will be the first track of the playlist. If index == playlist.size() the track will be added last. If inbetween, it’ll push tracks from the index onwards back in the playlist to make room.
///
/// \param completion Code to be executed upon completing the asynchronous request.
...
///
///
- (void)addTrackWithIdentifier:(NSString * _Nonnull)identifier metadata:(AutomixIQTrackModel * _Nullable)metadata index:(int32_t)index completion:(void (^ _Nullable)(NSError * _Nullable))completion;
...
/// AutomixIQArtistModel models an artist.
SWIFT_CLASS("_TtC9AutomixIQ20AutomixIQArtistModel")
@interface AutomixIQArtistModel : NSObject
/// The artist name.
...
- (nonnull instancetype)initWithApiKey:(NSString * _Nonnull)apiKey country:(NSString * _Nonnull)country storeId:(NSString * _Nonnull)storeId applicationId:(NSString * _Nonnull)applicationId automaticallyLogPlayback:(BOOL)automaticallyLogPlayback OBJC_DESIGNATED_INITIALIZER;
...
@property (nonatomic) BOOL automaticallyLogPlayback;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end

/// AutomixIQLogLevel is an enum of all the AutomixIQ available log levels
...
///
typedef SWIFT_ENUM(NSInteger, AutomixIQLogLevel, open) {
  AutomixIQLogLevelLogLevelTrace = 0,
  AutomixIQLogLevelLogLevelDebug = 1,
  AutomixIQLogLevelLogLevelInfo = 2,
  AutomixIQLogLevelLogLevelWarning = 3,
  AutomixIQLogLevelLogLevelError = 4,
  AutomixIQLogLevelLogLevelFatal = 5,
};
...
/// AutomixIQPluginMetadataModel models metadata of a track traditionally obtained by running plugins on it. Key and BPM are recommented for best mixing.
...
SWIFT_CLASS("_TtC9AutomixIQ28AutomixIQPluginMetadataModel")
@interface AutomixIQPluginMetadataModel : NSObject
...
///
- (void)logAutomixIQMessageWithLogLevel:(enum AutomixIQLogLevel)logLevel message:(NSString * _Nonnull)message;
@required
...
///
///
- (void)asyncGetTrackMetadataWithTrackId:(NSString * _Nonnull)trackId completion:(void (^ _Nonnull)(NSError * _Nullable, AutomixIQTrackModel * _Nullable))completion;
...
///
/// \param logPlayType A descriptor of the event, one of the following: “Start”, “5sec”, “30sec”, “Stop”, “Unknown”.
///
/// \param completion Code to be executed upon completing the asynchronous request
///
/// \a completion parameters:
...
///
///
- (void)asyncLogPlayEventWithTrackId:(NSString * _Nonnull)trackId seconds:(NSInteger)seconds guid:(NSString * _Nonnull)guid logPlayType:(NSString * _Nonnull)logPlayType completion:(void (^ _Nonnull)(BOOL))completion;
...
- (AutomixIQTrackModel * _Nullable)syncGetTrackMetadataWithTrackId:(NSString * _Nullable)trackId SWIFT_WARN_UNUSED_RESULT;
/// AutomixIQ calls this function to provide a chance to cache metadata to shortcut future requests.
- (void)cacheMetadata:(NSArray<AutomixIQTrackModel *> * _Nonnull)metadata;
@end


/// AutomixIQTrackModel models a track. This metadata is required.
SWIFT_CLASS("_TtC9AutomixIQ19AutomixIQTrackModel")
@interface AutomixIQTrackModel : NSObject
/// The artists of a track - the first one is the one to be presented by a UI
@property (nonatomic, copy) NSArray<AutomixIQArtistModel *> * _Nullable Artists;
...
SWIFT_CLASS("_TtC9AutomixIQ36AutomixIQTunedAuthenticationResponse")
@interface AutomixIQTunedAuthenticationResponse : NSObject
/// The access token
...
/// AutomixIQTunedMusicCacheDelegate protocol defines the methods to be implemented with storing and providing cached track data
SWIFT_PROTOCOL("_TtP9AutomixIQ32AutomixIQTunedMusicCacheDelegate_")
@protocol AutomixIQTunedMusicCacheDelegate <NSObject>
/// Called by AutomixIQ when a track has been fully downloaded. This can be used to cache the track locally on the user’s device.
...
/// returns:
/// the metadata of the track, nil if not available.
- (AutomixIQTrackModel * _Nullable)getTrackMetadataWithTrackId:(NSString * _Nullable)trackId SWIFT_WARN_UNUSED_RESULT;
@end


/// AutomixIQTunedRestDelegate protocol defines the methods to be implemented that will provide AutomixIQ the credentials and identifiers to Tuned Global’s backend
SWIFT_PROTOCOL("_TtP9AutomixIQ26AutomixIQTunedRestDelegate_")
@protocol AutomixIQTunedRestDelegate <NSObject>
