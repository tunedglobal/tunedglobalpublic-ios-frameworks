...
/// true if the EQ is attached, false otherwise
-(BOOL)attached;

/// Adds input parameters to the audio mix
/// If inputParameters is nil, only the EQ will remain and all other effects applied to the audio mix will be gone
-(void)appendInputParameters:(nullable NSMutableArray*)inputParameters;

/// Sets a volume ramp on the current audio track while preserving the EQ tap.
