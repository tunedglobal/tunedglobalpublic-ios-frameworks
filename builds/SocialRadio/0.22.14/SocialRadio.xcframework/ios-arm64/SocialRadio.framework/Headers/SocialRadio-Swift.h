...
@class NSString;
@class SocialRadioCredentials;
/// Handles handoff of authentication tokens
SWIFT_PROTOCOL("_TtP11SocialRadio22AuthenticationDelegate_")
@protocol AuthenticationDelegate
/// Refreshes the credentials
/// \param oldAccessToken The expuired access token.¨
///
...
///
///
- (void)refreshCredentialsWithOldAccessToken:(NSString * _Nullable)oldAccessToken tokenType:(NSString * _Nullable)tokenType completion:(void (^ _Nonnull)(NSError * _Nullable, SocialRadioCredentials * _Nullable, NSString * _Nullable))completion;
@end
...
///
/// \param afterTrackIdentifier The identifier of the track thatshould have an ad after, nil if it should be added after the currently playing track
///
/// \param completion Code to be executed upon completing the asynchronous request.
///
/// \a completion parameters:
...
///
///
- (void)addAdsWithMetadata:(NSArray<TunedAdModel *> * _Nonnull)metadata afterTrackIdentifier:(NSString * _Nullable)afterTrackIdentifier completion:(void (^ _Nullable)(NSError * _Nullable))completion;
...
/// \param bypassSeekTransition true if you want as accurate and quick jump, false if you want the best sounding jump
///
/// \param completion Code to be executed upon completing the asynchronous request.
///
/// \a completion parameters:
...
///
///
- (void)nextWithBypassSeekTransition:(BOOL)bypassSeekTransition completion:(void (^ _Nullable)(NSError * _Nullable))completion;
...
///
///
- (void)previousWithBypassSeekTransition:(BOOL)bypassSeekTransition completion:(void (^ _Nullable)(NSError * _Nullable))completion;
...
///
///
- (void)backWithBypassSeekTransition:(BOOL)bypassSeekTransition completion:(void (^ _Nullable)(NSError * _Nullable))completion;
...
///
///
- (void)seekToSeconds:(float)seconds bypassSeekTransition:(BOOL)bypassSeekTransition completion:(void (^ _Nullable)(NSError * _Nullable))completion;
...
/// \param completion Code to be executed upon completing the asynchronous request.
///
- (void)skipToTrackAtIndex:(NSInteger)index bypassSeekTransition:(BOOL)bypassSeekTransition completion:(void (^ _Nullable)(NSError * _Nullable))completion;
...
@class AutomixIQEqualiserConfig;
...
@property (nonatomic, strong) AutomixIQEqualiserConfig * _Nonnull equaliserConfig;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end

SWIFT_CLASS("_TtC11SocialRadio24AutomixIQEqualiserConfig")
@interface AutomixIQEqualiserConfig : NSObject
@property (nonatomic) float lowPassFrequency;
@property (nonatomic) float highPassFrequency;
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end
...
typedef SWIFT_ENUM(NSInteger, AutomixIQStatusVariable, open) {
  ...
  AutomixIQStatusVariableAdIndex = 33,
  AutomixIQStatusVariablePlayingAd = 34,
  AutomixIQStatusVariableAdTrackTotalTime = 35,
  AutomixIQStatusVariableAdTrackCurrentTime = 36,
};
...
typedef SWIFT_ENUM(NSInteger, ErrorCode, open) {
  ...
  ErrorCodeDidNotAddAdIndexPassedAlready = 1047,
};

@class SocialRadioUser;
/// (Required) Handles communication between SocialRadio SDK and a backend regarding authentication
SWIFT_PROTOCOL("_TtP11SocialRadio33SocialLayerAuthenticationDelegate_")
@protocol SocialLayerAuthenticationDelegate <AuthenticationDelegate>
/// Gets the logged in user, nil if no user is logged in.
///
/// returns:
/// the logged in user, nil if no user is logged in.
- (SocialRadioUser * _Nullable)loggedInUser SWIFT_WARN_UNUSED_RESULT;
/// Sets the logged in user
/// \param user the logged in user
///
- (void)setLoggedInUser:(SocialRadioUser * _Nullable)user;
@end

@protocol SocialLayerUserDelegate;
@protocol SocialLayerLiveMixtapeDelegate;
@protocol SocialLayerMixtapeLikesDelegate;
@protocol SocialLayerMixtapeCommentsDelegate;
@protocol SocialLayerMixtapeBackgroundsDelegate;
@protocol SocialLayerTrackLikesDelegate;
@protocol SocialLayerModerationDelegate;
@protocol SocialLayerNotificationsDelegate;
/// SocialLayerDelegates is an object that holds all delegates used to communicate between the SocialRadio SDK and a backend that handles storage of social interactions.
/// ‘authenticationDelegate’, ‘userDelegate’ and ‘liveMixtapeDelegate’ are required always. Every other delegate can be nil and is only required if your backend and SocialRadio implementation supports the attached features, like commenting, moderation, backgrounds, etc.
/// See each delegate’s documentation for details.
SWIFT_CLASS("_TtC11SocialRadio20SocialLayerDelegates")
@interface SocialLayerDelegates : NSObject
/// Handles communication between SocialRadio SDK and a backend regarding authentication
@property (nonatomic, strong) id <SocialLayerAuthenticationDelegate> _Nonnull authenticationDelegate;
/// Handles communication between SocialRadio SDK and a backend regarding users.
@property (nonatomic, strong) id <SocialLayerUserDelegate> _Nonnull userDelegate;
/// Handles communication between SocialRadio SDK and a backend regarding live mixtapes.
@property (nonatomic, strong) id <SocialLayerLiveMixtapeDelegate> _Nonnull liveMixtapeDelegate;
/// Handles communication between SocialRadio SDK and a backend regarding liking and unliking a mixtape.
@property (nonatomic, strong) id <SocialLayerMixtapeLikesDelegate> _Nullable mixtapeLikesDelegate;
/// Handles communication between SocialRadio SDK and a backend regarding commenting a mixtape.
@property (nonatomic, strong) id <SocialLayerMixtapeCommentsDelegate> _Nullable mixtapeCommentsDelegate;
/// Handles communication between SocialRadio SDK and a backend regarding mixtape backgrounds.
@property (nonatomic, strong) id <SocialLayerMixtapeBackgroundsDelegate> _Nullable mixtapeBackgroundsDelegate;
/// Handles communication between SocialRadio SDK and a backend regarding liking and unliking a track.
@property (nonatomic, strong) id <SocialLayerTrackLikesDelegate> _Nullable trackLikesDelegate;
/// Handles communication between SocialRadio SDK and a backend regarding content moderation.
@property (nonatomic, strong) id <SocialLayerModerationDelegate> _Nullable moderationDelegate;
/// Handles communication between SocialRadio SDK and a backend regarding notifications.
@property (nonatomic, strong) id <SocialLayerNotificationsDelegate> _Nullable notificationsDelegate;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end

@class SocialRadioMixtape;
/// Reports live show lifecycle so a host can update its live list incrementally instead of reloading
/// it. Entirely optional - leave it nil and nothing changes.
SWIFT_PROTOCOL("_TtP11SocialRadio30SocialLayerLiveMixListDelegate_")
@protocol SocialLayerLiveMixListDelegate <NSObject>
/// A live show started.
/// \param identifier The show’s mix ID.
///
/// \param mixtape The show, when SocialRadio has it. Nil is possible - a listener who never
/// saw the show created has no model for it.
///
- (void)liveShowStartedWithIdentifier:(NSString * _Nonnull)identifier mixtape:(SocialRadioMixtape * _Nullable)mixtape;
/// A live show ended. Any model SocialRadio held for it is released after this call.
/// \param identifier The show’s mix ID.
///
/// \param mixtape The show, when SocialRadio has it, nil otherwise.
///
- (void)liveShowEndedWithIdentifier:(NSString * _Nonnull)identifier mixtape:(SocialRadioMixtape * _Nullable)mixtape;
/// A live show was renamed. Also fires on a listener when a new title arrives with a recipe update.
/// \param identifier The show’s mix ID.
///
/// \param mixtape The show, when SocialRadio has it, nil otherwise.
///
...
///
- (void)liveShowRenamedWithIdentifier:(NSString * _Nonnull)identifier mixtape:(SocialRadioMixtape * _Nullable)mixtape newTitle:(NSString * _Nonnull)newTitle;
/// A user in the show changed their profile - a new display name, a new avatar, or both. Fires on
/// every participant, including the one who made the change. SocialRadio has already refreshed
/// what it draws; this is for a host that also shows the user somewhere of its own.
/// \param identifier The show’s mix ID.
///
/// \param mixtape The show, when SocialRadio has it, nil otherwise.
///
/// \param userIdentifier The user whose profile changed. Re-read the profile to see it - the
/// new name and avatar URL are deliberately not carried over the wire.
///
- (void)liveShowProfileChangedWithIdentifier:(NSString * _Nonnull)identifier mixtape:(SocialRadioMixtape * _Nullable)mixtape userIdentifier:(NSString * _Nonnull)userIdentifier;
@end

@class SocialRadioList;
/// (Required) Handles communication between SocialRadio SDK and a backend regarding live mixtapes.
SWIFT_PROTOCOL("_TtP11SocialRadio30SocialLayerLiveMixtapeDelegate_")
@protocol SocialLayerLiveMixtapeDelegate <NSObject>
/// Retrieves the recipe file
/// \param identifier The mixtape identifier.
///
/// \param completion Code to be executed upon completing the asynchronous request.
///
/// \a completion parameters:
...
/// </li>
/// <li>
/// recipe: the recipe file data.
/// </li>
/// </ul>
///
///
- (void)downloadRecipeWithIdentifier:(NSString * _Nonnull)identifier completion:(void (^ _Nonnull)(NSError * _Nullable, NSData * _Nullable))completion;
/// Retrieves the MQTT broker address for live sessions.
/// \param completion Code to be executed upon completing the asynchronous request.
///
/// \a completion parameters:
/// <ul>
...
///
///
- (void)getMqttBrokerAddressWithCompletion:(void (^ _Nonnull)(NSError * _Nullable, NSString * _Nullable))completion;
/// Fetches available live shows.
/// \param limit The maximum number of results to return.
///
/// \param start An optional cursor for paginated results.
///
/// \param completion Code to be executed upon completing the asynchronous request.
///
/// \a completion parameters:
...
/// </li>
/// <li>
/// liveMixtapes: The list of live mixtapes retrieved.
/// </li>
/// </ul>
///
///
- (void)getLiveShowsWithLimit:(NSInteger)limit start:(NSString * _Nullable)start completion:(void (^ _Nonnull)(NSError * _Nullable, SocialRadioList * _Nullable))completion;
/// Sends a keep-alive signal to maintain a live session.
...
///
///
- (void)postKeepAliveSessionWithIdentifier:(NSString * _Nonnull)identifier audience:(NSInteger)audience completion:(void (^ _Nonnull)(NSError * _Nullable))completion;
/// Ends a live session and optionally returns the published mix.
/// \param identifier The unique session identifier.
///
/// \param completion Code to be executed upon completing the asynchronous request.
///
...
/// </li>
/// <li>
/// publishedMix: The published mixtape, if any.
/// </li>
/// </ul>
///
///
- (void)postEndLiveSessionWithIdentifier:(NSString * _Nonnull)identifier completion:(void (^ _Nonnull)(NSError * _Nullable, SocialRadioMixtape * _Nullable))completion;
/// Retrieves the live mixtape of a specific user.
...
/// </li>
/// <li>
/// mixtape: The live mixtape, if found.
/// </li>
/// </ul>
///
///
- (void)getUserLiveMixtapeWithIdentifier:(NSString * _Nonnull)identifier completion:(void (^ _Nonnull)(NSError * _Nullable, SocialRadioMixtape * _Nullable))completion;
/// Creates a  mixtape.
/// \param mixDictionary Metadata and configuration for the mix.
///
/// \param isPrivate Flag indicating whether the mix is private.
///
/// \param recipeJSONString Optional JSON string for recipe data.
///
/// \param tracksJSONString Optional JSON string for track data.
///
/// \param mixHash The hash of the mix file
///
/// \param completion Code to be executed upon completing the asynchronous request.
///
/// \a completion parameters:
...
/// </li>
/// <li>
/// mix: The created <code>SocialRadioMixtape</code> object.
/// </li>
/// </ul>
///
///
- (void)postCreateMixtapeWithMixDictionary:(NSDictionary<NSString *, id> * _Nonnull)mixDictionary isPrivate:(BOOL)isPrivate recipeJSONString:(NSString * _Nullable)recipeJSONString tracksJSONString:(NSString * _Nullable)tracksJSONString mixHash:(NSUInteger)mixHash completion:(void (^ _Nonnull)(NSError * _Nullable, SocialRadioMixtape * _Nullable))completion;
/// Checks if a recipe is the latest version.
/// \param identifier The mixtape identifier.
///
/// \param mixHash The hash representing the current mix state.
///
/// \param completion Code to be executed upon completing the asynchronous request.
///
/// \a completion parameters:
...
///
///
- (void)getIsLatestRecipeWithIdentifier:(NSString * _Nonnull)identifier mixHash:(NSInteger)mixHash completion:(void (^ _Nonnull)(NSError * _Nullable))completion;
/// Attempts to commit a co-creation mixtape.
/// \param identifier The mixtape ID.
///
/// \param newTitle Optional new title for the mix.
///
/// \param recipe Base64 Encoded recipe string.
///
/// \param mixDictionary Updated mix metadata.
///
/// \param oldMixHash Previous hash of the mix.
///
/// \param newMixHash New hash after changes.
///
/// \param recipeJSONString The JSON string representing the recipe’s metadata.
///
/// \param tracksJSONString The JSON string representing the tracks in the recipe’s metadata..
///
/// \param completion Code to be executed upon completing the asynchronous request.
///
/// \a completion parameters:
...
///
///
- (void)postUpdateMixtapeMetadataWithIdentifier:(NSString * _Nonnull)identifier newTitle:(NSString * _Nullable)newTitle recipe:(NSData * _Nonnull)recipe mixDictionary:(NSDictionary<NSString *, id> * _Nonnull)mixDictionary oldMixHash:(NSUInteger)oldMixHash newMixHash:(NSUInteger)newMixHash recipeJSONString:(NSString * _Nullable)recipeJSONString tracksJSONString:(NSString * _Nullable)tracksJSONString completion:(void (^ _Nonnull)(NSError * _Nullable))completion;
/// Updates the recipe of a mixtape.
/// \param identifier The mixtape ID.
///
/// \param recipe The binary recipe data.
///
/// \param completion Code to be executed upon completing the asynchronous request.
///
/// \a completion parameters:
...
///
///
- (void)putUpdateMixtapeRecipeWithIdentifier:(NSString * _Nonnull)identifier recipe:(NSData * _Nonnull)recipe completion:(void (^ _Nonnull)(NSError * _Nullable))completion;
/// Retrieves a published mix.
/// \param identifier The ID of the published mix.
///
/// \param completion Code to be executed upon completing the asynchronous request.
///
/// \a completion parameters:
...
/// </li>
/// <li>
/// publishedMix: The retrieved published mix.
/// </li>
/// </ul>
///
///
- (void)getMixWithIdentifier:(NSString * _Nonnull)identifier completion:(void (^ _Nonnull)(NSError * _Nullable, SocialRadioMixtape * _Nullable))completion;
/// Synchronously retrieves the MQTT broker address.
///
/// returns:
/// The address string of the broker, or nil.
- (NSString * _Nullable)syncGetMqttBrokerAddress SWIFT_WARN_UNUSED_RESULT;
@end

@class UIImage;
/// Handles communication between SocialRadio SDK and a backend regarding mixtape backgrounds.
SWIFT_PROTOCOL("_TtP11SocialRadio37SocialLayerMixtapeBackgroundsDelegate_")
@protocol SocialLayerMixtapeBackgroundsDelegate <NSObject>
/// Retrieves a menu of available background images.
/// \param completion Code to be executed upon completion.
///
/// \a completion parameters:
/// <ul>
/// <li>
/// idsAndImages: A dictionary of background UUIDs and their corresponding images.
/// </li>
/// <li>
/// ids: A list of background UUIDs.
/// </li>
/// </ul>
///
///
- (void)getMenuWithCompletion:(SWIFT_NOESCAPE void (^ _Nonnull)(NSDictionary<NSString *, UIImage *> * _Nonnull, NSArray<NSString *> * _Nonnull))completion;
/// Retrieves a specific background image.
/// \param id The UUID of the background.
///
/// \param completion Code to be executed upon completion.
///
/// \a completion parameters:
...
/// </li>
/// <li>
/// uuid: The UUID of the retrieved background.
/// </li>
/// <li>
/// image: The retrieved background image.
/// </li>
/// </ul>
///
///
- (void)getBackgroundWithId:(NSString * _Nonnull)id completion:(void (^ _Nonnull)(NSError * _Nullable, NSString * _Nullable, UIImage * _Nullable))completion;
/// Uploads a background image.
/// \param loggedInUserid The identifier of the logged in user
///
...
/// </li>
/// <li>
/// uuid: The UUID assigned to the uploaded image.
/// </li>
/// </ul>
///
///
- (void)postUploadMixtapeBackgroundWithLoggedInUserid:(NSString * _Nonnull)loggedInUserid image:(UIImage * _Nonnull)image completion:(void (^ _Nonnull)(NSError * _Nullable, NSString * _Nullable))completion;
/// Retrieves a new user-specific background image.
/// \param completion Code to be executed upon completion.
///
/// \a completion parameters:
/// <ul>
/// <li>
/// uuid: The UUID of the new background.
/// </li>
/// <li>
...
///
///
- (void)getNewUserImageWithCompletion:(void (^ _Nonnull)(NSString * _Nullable, UIImage * _Nullable))completion;
/// Deletes a user’s background.
/// \param userIdentifier The user’s ID.
///
/// \param backgroundIdentifier The UUID of the background to delete.
///
/// \param completion Code to be executed upon completion.
///
/// \a completion parameters:
...
///
///
- (void)deleteUserBackgroundWithUserIdentifier:(NSString * _Nonnull)userIdentifier backgroundIdentifier:(NSString * _Nonnull)backgroundIdentifier completion:(void (^ _Nonnull)(NSError * _Nullable))completion;
/// Retrieves a list of background identifiers for the logged-in user.
/// \param userIdentifier The user’s ID.
///
/// \param completion Code to be executed upon completion.
///
...
/// </li>
/// <li>
/// ids: List of background UUIDs.
/// </li>
/// </ul>
///
///
- (void)getLoggedInUserBackgroundIdentifiersWithUserIdentifier:(NSString * _Nonnull)userIdentifier completion:(void (^ _Nonnull)(NSError * _Nullable, NSArray<NSString *> * _Nullable))completion;
/// Synchronously retrieves a cached background image.
/// \param backgroundIdentifier The identifier of the background.
///
///
/// returns:
/// The cached background image, or nil.
- (UIImage * _Nullable)syncGetImageWithBackgroundIdentifier:(NSString * _Nonnull)backgroundIdentifier SWIFT_WARN_UNUSED_RESULT;
/// Checks whether a background is preloaded.
/// \param backgroundIdentifier The identifier of the background.
///
///
/// returns:
/// <code>true</code> if preloaded, <code>false</code> otherwise.
- (BOOL)syncGetIsPreloadedWithBackgroundIdentifier:(NSString * _Nonnull)backgroundIdentifier SWIFT_WARN_UNUSED_RESULT;
/// Preloads all backgrounds associated with the logged-in user.
/// \param userIdentifier The ID of the user.
///
- (void)syncPreloadLoggedInUserBackgroundsWithUserIdentifier:(NSString * _Nonnull)userIdentifier;
@end

@class SocialRadioComment;
@class SocialRadioCommentTag;
/// Handles communication between SocialRadio SDK and a backend regarding commenting a mixtape.
SWIFT_PROTOCOL("_TtP11SocialRadio34SocialLayerMixtapeCommentsDelegate_")
@protocol SocialLayerMixtapeCommentsDelegate <NSObject>
/// Fetches general or timeline-specific comments on a mixtape.
/// \param identifier The unique ID of the mixtape.
///
/// \param timelineComments Flag indicating whether to fetch timeline comments only.
///
/// \param completion Code to be executed upon completing the asynchronous request.
///
/// \a completion parameters:
...
///
///
- (void)getCommentsWithIdentifier:(NSString * _Nonnull)identifier timelineComments:(BOOL)timelineComments completion:(void (^ _Nonnull)(NSError * _Nullable, NSArray<SocialRadioComment *> * _Nullable))completion;
/// Posts a comment to a mixtape.
/// \param userIdentifier ID of the user posting the comment.
///
/// \param mixIdentifier ID of the mixtape.
///
/// \param text Comment text.
///
/// \param tags List of tags for the comment.
///
/// \param trackIndex10 Optional track index.
///
/// \param mixTimestamp Optional timestamp in the mix.
///
/// \param completion Code to be executed upon completing the asynchronous request.
///
/// \a completion parameters:
...
/// </li>
/// <li>
/// comment: The posted comment object.
/// </li>
/// </ul>
///
///
- (void)postCommentWithUserIdentifier:(NSString * _Nonnull)userIdentifier mixIdentifier:(NSString * _Nonnull)mixIdentifier text:(NSString * _Nonnull)text tags:(NSArray<SocialRadioCommentTag *> * _Nonnull)tags trackIndex10:(NSNumber * _Nullable)trackIndex10 mixTimestamp:(NSNumber * _Nullable)mixTimestamp completion:(void (^ _Nonnull)(NSError * _Nullable, SocialRadioComment * _Nullable))completion;
/// Retrieves a specific comment from a mixtape.
/// \param mixIdentifier ID of the mixtape.
///
/// \param commentIdentifier ID of the comment.
///
/// \param completion Code to be executed upon completing the asynchronous request.
///
/// \a completion parameters:
...
///
///
- (void)getCommentWithMixIdentifier:(NSString * _Nonnull)mixIdentifier commentIdentifier:(NSString * _Nonnull)commentIdentifier completion:(void (^ _Nonnull)(NSError * _Nullable, SocialRadioComment * _Nullable))completion;
/// Deletes a specific comment from a mixtape.
/// \param mixIdentifier ID of the mixtape.
///
/// \param commentIdentifier ID of the comment to delete.
///
/// \param completion Code to be executed upon completing the asynchronous request.
///
/// \a completion parameters:
...
///
///
- (void)deleteCommentWithMixIdentifier:(NSString * _Nonnull)mixIdentifier commentIdentifier:(NSString * _Nonnull)commentIdentifier completion:(void (^ _Nonnull)(NSError * _Nullable))completion;
@end

/// Handles communication between SocialRadio SDK and a backend regarding liking and unliking a mixtape.
SWIFT_PROTOCOL("_TtP11SocialRadio31SocialLayerMixtapeLikesDelegate_")
@protocol SocialLayerMixtapeLikesDelegate <NSObject>
/// Likes a mixtape.
/// \param identifier The unique ID of the mixtape to like.
///
/// \param completion Code to be executed upon completing the asynchronous request.
///
/// \a completion parameters:
...
///
///
- (void)postLikeMixtapeWithIdentifier:(NSString * _Nonnull)identifier completion:(void (^ _Nonnull)(NSError * _Nullable))completion;
/// Unlikes a mixtape.
/// \param identifier The unique ID of the mixtape to unlike.
///
/// \param completion Code to be executed upon completing the asynchronous request.
///
/// \a completion parameters:
...
///
///
- (void)postUnlikeMixtapeWithIdentifier:(NSString * _Nonnull)identifier completion:(void (^ _Nonnull)(NSError * _Nullable))completion;
/// Retrieves the list of likes for a mixtape.
/// \param identifier The unique ID of the mixtape.
///
/// \param limit Maximum number of likes to retrieve.
///
/// \param start Optional pagination cursor.
///
/// \param completion Code to be executed upon completing the asynchronous request.
///
/// \a completion parameters:
...
/// </li>
/// <li>
/// likes: The list of likes returned.
/// </li>
/// </ul>
///
///
- (void)getMixtapeLikesWithIdentifier:(NSString * _Nonnull)identifier limit:(NSInteger)limit start:(NSString * _Nullable)start completion:(void (^ _Nonnull)(NSError * _Nullable, SocialRadioList * _Nullable))completion;
/// Checks if a mixtape is liked.
/// \param identifier The unique ID of the mixtape.
///
/// \param completion Code to be executed upon completing the asynchronous request.
///
...
/// </li>
/// <li>
/// isLiked: A boolean indicating whether the mixtape is liked.
/// </li>
/// </ul>
///
///
- (void)getIsMixtapeLikedWithIdentifier:(NSString * _Nonnull)identifier completion:(void (^ _Nonnull)(NSError * _Nullable, BOOL))completion;
@end

/// Handles communication between SocialRadio SDK and a backend regarding content moderation.
SWIFT_PROTOCOL("_TtP11SocialRadio29SocialLayerModerationDelegate_")
@protocol SocialLayerModerationDelegate <NSObject>
/// Reports a user.
/// \param identifier The ID of the user to report.
///
/// \param completion Code to be executed upon completion.
///
/// \a completion parameters:
...
///
///
- (void)postReportUserWithIdentifier:(NSString * _Nonnull)identifier completion:(void (^ _Nullable)(NSError * _Nullable))completion;
@end

/// Handles communication between SocialRadio SDK and a backend regarding notifications.
SWIFT_PROTOCOL("_TtP11SocialRadio32SocialLayerNotificationsDelegate_")
@protocol SocialLayerNotificationsDelegate <NSObject>
/// Sends a notification to the user’s followers.
/// \param userIdentifier The ID of the user initiating the notification.
///
/// \param title The title of the notification.
///
/// \param body The body content of the notification.
///
/// \param objectType The type of object (e.g., mixtape, comment).
///
...
///
///
- (void)postNotifyFollowersWithUserIdentifier:(NSString * _Nonnull)userIdentifier title:(NSString * _Nonnull)title body:(NSString * _Nonnull)body objectType:(NSString * _Nonnull)objectType objectId:(NSString * _Nonnull)objectId completion:(void (^ _Nullable)(NSError * _Nullable))completion;
@end

/// Handles communication between SocialRadio SDK and a backend regarding liking and unliking a track.
SWIFT_PROTOCOL("_TtP11SocialRadio29SocialLayerTrackLikesDelegate_")
@protocol SocialLayerTrackLikesDelegate <NSObject>
/// Asynchronously checks if a track is liked.
/// \param identifier The unique ID of the track.
///
/// \param completion Code to be executed upon completion.
///
/// \a completion parameters:
/// <ul>
/// <li>
/// isLiked: Boolean indicating whether the track is liked.
/// </li>
/// </ul>
///
///
- (void)getIsTrackLikedWithIdentifier:(NSString * _Nonnull)identifier completion:(void (^ _Nonnull)(BOOL))completion;
/// Sets the like status of a track.
/// \param identifier The unique ID of the track.
///
/// \param liked Boolean indicating desired like state.
///
/// \param completion Code to be executed upon completion.
///
/// \a completion parameters:
...
///
///
- (void)postSetTrackLikedWithIdentifier:(NSString * _Nonnull)identifier liked:(BOOL)liked completion:(void (^ _Nonnull)(NSError * _Nullable, BOOL))completion;
/// Synchronously checks if a track is liked.
/// \param identifier The track ID.
///
///
/// returns:
/// A boolean wrapped in NSNumber indicating if the track is liked.
- (NSNumber * _Nullable)syncIsTrackLikedWithIdentifier:(NSString * _Nonnull)identifier SWIFT_WARN_UNUSED_RESULT;
/// Clears the local cache of liked tracks.
- (void)syncClearLikedTracksCache;
@optional
/// Asks the backend for the whole set of liked tracks so the cache is coherent with changes made
/// outside SocialRadio. Implementations are expected to rate limit themselves - the SDK calls
/// this every time the mix view is presented.
- (void)prefetchLikedTracks;
@end

/// (Required) Handles communication between SocialRadio SDK and a backend regarding  users.
SWIFT_PROTOCOL("_TtP11SocialRadio23SocialLayerUserDelegate_")
@protocol SocialLayerUserDelegate <NSObject>
/// Fetches a user given their unique identifier.
/// \param identifier The unique ID of the user to fetch.
///
/// \param completion Code to be executed upon completing the asynchronous request.
///
/// \a completion parameters:
...
/// </li>
/// <li>
/// user: The fetched <code>SocialRadioUser</code> object, if successful.
/// </li>
/// </ul>
///
///
- (void)getUserWithIdentifier:(NSString * _Nonnull)identifier completion:(void (^ _Nonnull)(NSError * _Nullable, SocialRadioUser * _Nullable))completion;
/// Follows a user on behalf of the logged-in user.
/// \param identifier The ID of the user to follow.
///
/// \param loggedInUserIdentifier The ID of the logged-in user initiating the follow.
///
/// \param completion Code to be executed upon completing the asynchronous request.
///
/// \a completion parameters:
...
///
///
- (void)postFollowUserWithIdentifier:(NSString * _Nonnull)identifier loggedInUserIdentifier:(NSString * _Nonnull)loggedInUserIdentifier completion:(void (^ _Nonnull)(NSError * _Nullable))completion;
/// Unfollows a user on behalf of the logged-in user.
/// \param identifier The ID of the user to unfollow.
///
/// \param loggedInUserIdentifier The ID of the logged-in user initiating the unfollow.
///
/// \param completion Code to be executed upon completing the asynchronous request.
///
/// \a completion parameters:
...
///
///
- (void)postUnfollowUserWithIdentifier:(NSString * _Nonnull)identifier loggedInUserIdentifier:(NSString * _Nonnull)loggedInUserIdentifier completion:(void (^ _Nonnull)(NSError * _Nullable))completion;
/// Retrieves a list of followers for a user.
/// \param identifier The ID of the user whose followers are being fetched.
///
/// \param limit The maximum number of results to return.
///
/// \param start An optional cursor for paginated results.
...
/// </li>
/// <li>
/// followers: The list of followers retrieved.
/// </li>
/// </ul>
///
///
- (void)getFollowersWithIdentifier:(NSString * _Nonnull)identifier limit:(NSInteger)limit start:(NSString * _Nullable)start completion:(void (^ _Nonnull)(NSError * _Nullable, SocialRadioList * _Nullable))completion;
/// Retrieves a list of users that the specified user is following.
/// \param identifier The ID of the user whose following list is being fetched.
///
/// \param limit The maximum number of results to return.
///
/// \param start An optional cursor for paginated results.
...
/// </li>
/// <li>
/// followers: The list of followed users retrieved.
/// </li>
/// </ul>
///
///
- (void)getFollowingIdentifier:(NSString * _Nonnull)identifier limit:(NSInteger)limit start:(NSString * _Nullable)start completion:(void (^ _Nonnull)(NSError * _Nullable, SocialRadioList * _Nullable))completion;
/// Retrieves follow statuses for a list of users.
/// \param identifiers List of user identifiers to check.
///
/// \param loggedInUserIdentifier The ID of the logged-in user.
///
/// \param completion Code to be executed upon completing the asynchronous request.
///
/// \a completion parameters:
...
/// </li>
/// <li>
/// userFollows: A mapping from user ID to follow status.
/// </li>
/// </ul>
///
///
- (void)getUserFollowsWithIdentifiers:(NSArray<NSString *> * _Nonnull)identifiers loggedInUserIdentifier:(NSString * _Nonnull)loggedInUserIdentifier completion:(void (^ _Nonnull)(NSError * _Nullable, NSDictionary<NSString *, id> * _Nullable))completion;
/// Retrieves a user’s avatar image.
/// \param identifier The ID of the user whose avatar is to be fetched.
///
/// \param completion Code to be executed upon completing the asynchronous request.
///
/// \a completion parameters:
...
/// </li>
/// <li>
/// image: The retrieved avatar image.
/// </li>
/// </ul>
///
///
- (void)getAvatarImageWithIdentifier:(NSString * _Nonnull)identifier completion:(void (^ _Nonnull)(NSError * _Nullable, UIImage * _Nullable))completion;
/// Searches for users based on a query string.
/// \param query The search query string.
///
/// \param mixIdentifier Optional mix context for filtering.
///
/// \param limit The maximum number of results to return.
///
/// \param completion Code to be executed upon completing the asynchronous request.
...
/// </li>
/// <li>
/// list: The resulting list of users.
/// </li>
/// </ul>
///
///
- (void)getUsersFromSearchWithQuery:(NSString * _Nonnull)query mixIdentifier:(NSString * _Nullable)mixIdentifier limit:(NSInteger)limit completion:(void (^ _Nonnull)(NSError * _Nullable, SocialRadioList * _Nullable))completion;
@end

@class SocialRadioConfig;
@protocol SocialRadioStatusObserverDelegate;
@protocol SocialRadioUIDelegate;
@protocol SocialRadioMetricsDelegate;
...
/// Initialiser for SocialRadio to work against a Tuned Global’s Store and Tuned Global’s APIs.
...
///
/// \param socialRadioTrackMetadataDelegate The delegate in charge of fetching artwork and recommendations for tracks
///
/// \param socialRadioStatusDelegate The delegate in charge of updating any UI with status updates from SocialRadio. See SocialRadioStatusObserverDelegate.
///
/// \param socialRadioUIDelegate The delegate in charge of presenting user interfaces the SocialRadio needs
///
/// \param authenticationDelegate The delegate to handle credentials handoff
///
/// \param completion Code to be executed upon completing the asynchronous request.
///
/// \a completion parameters:
...
///
///
+ (void)initialiseWithConfig:(SocialRadioConfig * _Nonnull)config automixIQStreamingDelegate:(id <AutomixIQStreamingDelegate> _Nonnull)automixIQStreamingDelegate socialRadioTrackMetadataDelegate:(id <SocialRadioTrackMetadataDelegate> _Nonnull)socialRadioTrackMetadataDelegate socialRadioStatusDelegate:(id <SocialRadioStatusObserverDelegate> _Nullable)socialRadioStatusDelegate socialRadioUIDelegate:(id <SocialRadioUIDelegate> _Nonnull)socialRadioUIDelegate authenticationDelegate:(id <AuthenticationDelegate> _Nonnull)authenticationDelegate liveMixListDelegate:(id <SocialLayerLiveMixListDelegate> _Nullable)liveMixListDelegate metricsDelegate:(id <SocialRadioMetricsDelegate> _Nullable)metricsDelegate completion:(void (^ _Nullable)(NSError * _Nullable))completion;
...
/// Returns wether Social Radio is actively managing the MPNowPlayingDictionary and Now Playing widget commands, as set by the feature set config object.
/// This can only be true if a radio session is ongoing.
+ (BOOL)isManagingNowPlayingDictionary SWIFT_WARN_UNUSED_RESULT;
/// Gets the Tuned Global’s backend implementation of the Social Layer delegates
///
/// returns:
/// The Tuned Global’s backend implementation of the Social Layer delegates
- (SocialLayerDelegates * _Nullable)tunedGlobalSocialLayerDelegates SWIFT_WARN_UNUSED_RESULT;
/// Sets the Social Layer delegates
/// \param delegates The social layer delegates
///
- (void)setSocialLayerDelegates:(SocialLayerDelegates * _Nullable)delegates;
...
///
///
- (void)startRadioSessionWithIdentifiers:(NSArray<NSString *> * _Nonnull)identifiers metadata:(NSArray<TunedTrackModel *> * _Nullable)metadata playTrackIndex:(int32_t)playTrackIndex seconds:(float)seconds autoplay:(BOOL)autoplay contextType:(NSString * _Nullable)contextType contextId:(int32_t)contextId completion:(void (^ _Nullable)(NSError * _Nullable, UIViewController * _Nullable))completion;
...
/// Tells SocialRadio that a user’s profile changed, so anything it is already showing for that
/// user - the avatar on a live show, on the shows list, against a comment - is refreshed rather
/// than served from the copy it took the first time.
/// \param identifier The identifier of the user whose profile changed
///
/// \param profile The new profile, when the caller already has it. Passed in, it is used
/// as-is and no name lookup is made; omitted, the name is fetched again on next use. The
/// avatar image is re-downloaded either way - the URL is always read fresh.
///
- (void)userProfileChangedWithIdentifier:(NSString * _Nonnull)identifier profile:(SocialRadioUser * _Nullable)profile;
/// Starts playing a live mixtape as a consumer
/// \param identifier The identifier of the user to join
///
/// \param completion Code to be executed upon completing the asynchronous request.
///
/// \a completion parameters:
...
///
///
- (void)joinLiveUserWithIdentifier:(NSString * _Nonnull)identifier joinMethod:(NSString * _Nullable)joinMethod completion:(void (^ _Nonnull)(NSError * _Nullable, UIViewController * _Nullable))completion;
/// Starts playing a live mixtape as a consumer
/// \param identifier The identifier of mixtape to join
///
/// \param joinMethod How the listener reached this show (‘link’, ‘feed’, ‘profile’,
/// ‘notification’, …), reported verbatim as join_method on the broadcast_join metrics
/// event. Free form: the SDK stores and echoes it, it does not interpret it. Can be nil.
///
/// \param completion Code to be executed upon completing the asynchronous request.
///
/// \a completion parameters:
...
///
///
- (void)joinLiveBroadcastWithIdentifier:(NSString * _Nonnull)identifier joinMethod:(NSString * _Nullable)joinMethod completion:(void (^ _Nonnull)(NSError * _Nullable, UIViewController * _Nullable))completion;
...
@class NSDate;
/// SocialRadioComment models a comment
SWIFT_CLASS("_TtC11SocialRadio18SocialRadioComment")
@interface SocialRadioComment : NSObject
/// The comment’s identifier
@property (nonatomic, copy) NSString * _Nullable commentId;
/// The comment’s mix Id
...
@property (nonatomic, readonly) NSInteger iserIdentifier;
/// The moment the comment was created
@property (nonatomic, copy) NSDate * _Nullable created;
/// The tags in the comment
@property (nonatomic, copy) NSArray<SocialRadioCommentTag *> * _Nullable tags;
/// Comment’s metadata
...
/// SocialRadioCommentTag models a user tag in a comment
SWIFT_CLASS("_TtC11SocialRadio21SocialRadioCommentTag")
@interface SocialRadioCommentTag : NSObject
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end
...
/// SocialRadioConfig holds the variables to initialise SocialRadio and describe the desired feature set and UI
...
/// Social Radio’s feature set options
@property (nonatomic, strong) SocialRadioFeatureSetConfig * _Nonnull featureSetConfig;
/// Social Radio UI options
@property (nonatomic, strong) SocialRadioUIConfig * _Nonnull uiConfig;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end

/// SocialRadioCredentials models credentials to access social REST APIs.
SWIFT_CLASS("_TtC11SocialRadio22SocialRadioCredentials")
@interface SocialRadioCredentials : NSObject
/// The access token.
...
@property (nonatomic) BOOL managesNowPlayingDictionary;
...
/// SocialRadio’s available log levels
...
enum SocialRadioMetricsEvent : NSInteger;
/// Implement this to forward SocialRadio’s broadcast analytics to the metrics platform of your
/// choice. Supplying it is optional; when no delegate is set nothing is reported and nothing is
/// retained. Metadata values are String, Int or Bool, keyed by the field names in the event spec.
SWIFT_PROTOCOL("_TtP11SocialRadio26SocialRadioMetricsDelegate_")
@protocol SocialRadioMetricsDelegate
/// Called on the main thread when a broadcast analytics event occurs.
/// \param event The event that occurred.
///
/// \param metadata The event’s fields. Which keys are present depends on the event.
///
- (void)reportEvent:(enum SocialRadioMetricsEvent)event metadata:(NSDictionary<NSString *, id> * _Nonnull)metadata;
@end

/// The analytics events SocialRadio reports. Broadcast create, start and end are reported by the
/// creator’s device; join, leave, listen milestone and rejoin are reported by each listener’s.
typedef SWIFT_ENUM(NSInteger, SocialRadioMetricsEvent, open) {
  SocialRadioMetricsEventBroadcastCreate = 0,
  SocialRadioMetricsEventBroadcastStart = 1,
  SocialRadioMetricsEventBroadcastEnd = 2,
  SocialRadioMetricsEventBroadcastJoin = 3,
  SocialRadioMetricsEventBroadcastLeave = 4,
  SocialRadioMetricsEventBroadcastListenMilestone = 5,
  SocialRadioMetricsEventBroadcastRejoin = 6,
};
...
/// The minimum version of the Mix Engine that can play this mixtape
...
/// True if this track is an ad, false otherwise
...
/// Handles track recommendations and artwork
SWIFT_PROTOCOL("_TtP11SocialRadio32SocialRadioTrackMetadataDelegate_")
@protocol SocialRadioTrackMetadataDelegate <NSObject>
...
@property (nonatomic, strong) UIColor * _Nonnull dialogSwitchOnColor;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
...
///
///
- (void)presentTracklistWithFilterOutExplicitContent:(BOOL)filterOutExplicitContent completion:(void (^ _Nonnull)(NSArray<TunedTrackModel *> * _Nullable, NSString * _Nullable, NSInteger, NSString * _Nullable))completion;
@optional
...
- (nonnull instancetype)initWithUserIdentifier:(NSString * _Nonnull)userIdentifier alias:(NSString * _Nullable)alias userThumbnail:(NSString * _Nullable)userThumbnail OBJC_DESIGNATED_INITIALIZER;
/// The user’s alias
...
SWIFT_CLASS("_TtC11SocialRadio12TunedAdModel")
...
@property (nonatomic) NSInteger adDuration;
...
SWIFT_CLASS("_TtC11SocialRadio18TunedStoreSettings")
